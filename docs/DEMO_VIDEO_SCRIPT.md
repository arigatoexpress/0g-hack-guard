...
|------|------------------------|------------------|
| **0:00–0:15** | *"April 2026. In 0guard's source-linked dataset, twenty-eight DeFi incidents total about six hundred and thirty-five million dollars in reported losses. The scary part is that many were not exotic smart-contract bugs. They were operational-security failures: social engineering, bridge misconfigs, and stolen deployer keys."* | **Full-screen terminal.** Run `python3 scripts/demo_april_2026.py` in a pre-sized terminal (no output yet). Overlay a subtle text card in top-right corner: `April 2026: $635M+ reported losses`. Use dark background, monospace font. Pause on the command before hitting Enter. |
| **0:15–0:28** | *"Today, AI agents trade, bridge, and sign transactions in milliseconds. One bad prompt, one forged message, and an autonomous agent can hand a wallet to a hacker before a human even blinks. Few tools check agent intent before the signer."* | **Split screen:** Left = news headlines about April 2026 hacks (static image or browser tabs). Right = a short Python snippet from `crypto_hack_guard.py` showing the `KNOWN_MALICIOUS_ADDRESSES` list. Slow zoom into the Drift Protocol address. |
...
| **2:35–2:50** | *"The evidence layer is not a screenshot. The provenance matrix matches all twenty-eight April incidents to reviewed public-source evidence, keeps record hashes, and now shows full detector coverage after the Quant EIP-7702 promotion. Live refresh is separate, so the demo stays reliable."* | **Dashboard.** Click `Provenance` in Data Flow, then the signature map. Zoom into `28/28`, `1.0`, `Quant`, `EIP-7702`, and `rawPayloadsReturned: false`. |
...
| 2:35 | Zoom 1.5× on provenance `28/28`, detector map `28/28`, `1.0`, and `Quant` |
...
- **28** — incidents
- **Attribution** — public sources tie the largest April loss clusters to Lazarus Group
