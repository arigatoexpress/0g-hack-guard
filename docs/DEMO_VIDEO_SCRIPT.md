...
| **2:35–2:50** | *"The evidence layer is not a screenshot. The provenance matrix matches all twenty-eight April incidents to reviewed public-source evidence, keeps record hashes, and shows the remaining detector gap honestly. Live refresh is separate, so the demo stays reliable."* | **Dashboard.** Click `Provenance` in Data Flow, then the signature map. Zoom into `28/28`, `27/28`, `0.9643`, `Quant`, and `rawPayloadsReturned: false`. |
...
| 2:35 | Zoom 1.5× on provenance `28/28`, detector map `27/28`, `0.9643`, and `Quant` |
